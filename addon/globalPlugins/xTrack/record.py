...
class Recorder:
	...
	def _play_count_in_async(self, on_complete):
		beep_sequence = [(440, 200), (440, 200), (440, 200), (880, 200)]
		
		def play_beep(index=0):
			if index >= len(beep_sequence):
				core.callLater(0, on_complete)
				return
			freq, duration = beep_sequence[index]
			tones.beep(freq, duration)
			core.callLater(500, play_beep, index + 1)
		
		play_beep()

	def _setup_backend_recorder(self):
		global BACKEND_AVAILABLE, WasapiSoundRecorder
	...
	def start(self):
		...
		if conf.get("countIn", False):
			self._play_count_in_async(self._do_start_recording)
		else:
...
class RecordSettingsDialog(wx.Dialog):
	...
	def makeSettings(self):
		...
		if BACKEND_AVAILABLE:
			...
			status_text += _("Not available - check tools/recorder_backend.py")
