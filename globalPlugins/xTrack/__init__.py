...
# Import record module after config and paths are ready
from . import record
...
class GlobalPlugin(globalPluginHandler.GlobalPlugin):
    ...
    def __init__(self):
        ...
        # Ensure tool paths are accessible to the class instance
    ...
    def getSelectedFiles(self):
        """Get selected files from File Explorer using COM API."""
        try:
            fg = api.getForegroundObject()
            if not (fg.appModule and fg.appModule.appName == "explorer"):
                log.info("Not in Explorer")
                return []
           
            log.info(f"In Explorer, windowHandle: {fg.windowHandle}")
            ...
            for window in shell.Windows():
                try:
                    if window.hwnd == fg.windowHandle:
                        ...
                        log.info("Found matching Explorer window via COM")
                        break
                except Exception:
                    continue
           
            if not target_window:
                log.warning("Could not find matching Explorer window via COM")
                return []
            ...
            try:
                ...
                if selected_items:
                    log.info(f"Found {selected_items.Count} selected items")
                        ...
                        try:
                            ...
                            if hasattr(item, 'Path'):
                                ...
                                if path and os.path.exists(path):
                                    paths.append(path)
                                    log.info(f"Selected item path: {path}")
                        except Exception as e:
            ...
            if not paths:
                try:
                    ...
                    if focused_item:
                        if hasattr(focused_item, 'Path'):
                            path = focused_item.Path
                            if path and os.path.exists(path):
                                paths.append(path)
                                log.info(f"Focused item path: {path}")
                except Exception as e:
            ...
            if not paths:
                try:
                    ...
                    if hasattr(folder, 'Self'):
                        ...
                        log.info(f"Current folder: {folder_path}")
                        ...
                        if hasattr(focused, 'name') and focused.name:
                            ...
                            if os.path.exists(full_path):
                                ...
                                log.info(f"Constructed path from folder and focused object: {full_path}")
                except Exception as e:
